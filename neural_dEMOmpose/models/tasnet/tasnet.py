...
class TasNet(nn.Module):
    '''
        ...
        Args:
            ...
            X: int, number of conv blocks in each repeat (max depth
                    of dilations)
    ...
    def __init__(self, N=256, L=20, B=256, H=512, P=3, X=8, R=4, C=2):
        ...
        #  self.decoder = nn.ConvTranspose2d(C, C, (N, L),
        #                 stride=(1, L//2), groups=C)

    def forward(self, x):
        x = self.encoder(x)
        m_out = self.sep_module(x)
        #  x_hat = self.decoder(m_out)
        x_hat = torch.cat([self.decoder(m_out[:, c, :, :])
                           for c in range(m_out.shape[1])], dim=1)
        return x_hat
...
if __name__ == '__main__':
    ...
    print('==> Number of trainable params: {}'.format(sum(p.numel()
